...
fn debug_request(text: &String, name: &str) {
    // let mut file = File::create(format!("{}-{}.txt", name, debug_count)).unwrap();
    // debug_count += 1;
...
impl TsbContainer {
    pub fn load_creds() -> Result<Self, MissingUsernamePassword> {
        let mut file =
            File::open("creds.txt").map_err(|e| MissingUsernamePassword::FileIoError(e))?;
        ...
        .filter_map(|elm| {
            match elm {
                FormElement::Input { name, id, value } => {
                    if name == &Some("nextSequenceID".to_owned())
                        || id == &Some("nextSequenceID".to_owned())
                    {
                        return value.clone();
                    }
                }
